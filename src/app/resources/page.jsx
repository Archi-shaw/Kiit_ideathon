...
const Resources = () => {
  ...
  return (
    <div className="h-full mt-8 flex flex-col items-center justify-center">
      <h1 className="text-black dark:text-white font-bold text-3xl">Select Your Branch</h1>
      
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2 w-full m-2 p-2 cursor-pointer">
        {branches.map((branch) => (
          <div
            key={branch}
            className="bg-light-background dark:bg-dark-background h-12 rounded-md font-bold flex items-center justify-center shadow-md
                       transition-all duration-300 ease-in-out transform hover:bg-accent hover:scale-105 hover:shadow-xl"
            onClick={() => { setSelectedBranch(branch); setSelectedSemester(null); }} // Reset semester on branch selection
          >
            {branch}
          </div>
        ))}
      ...
      {selectedBranch && (
        <>
          <h1 className="text-black dark:text-white font-bold text-2xl mt-4">
      ...
      {selectedBranch && selectedSemester && subjects && (
        <div className="mt-6 w-full">
          ...
          <ul className="list-disc ml-5 mt-2 text-black dark:text-white">
            {subjects[selectedBranch][selectedSemester].map((subject, index) => (
              <li key={index} className="my-2">{subject}</li>
            ))}
          </ul>
        </div>
      )}
